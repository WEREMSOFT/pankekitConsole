...
void platformReadInput(StcGame *gameInstance) {
  int actualPotRead = map(analogRead(A5), 0, 1024, -5, 15);
#define LEFTBIT 1
#define RIGHTBIT 2
#define UPBIT 4
#define DOWNBIT 8
...
#define PSELECT (prev & SELECTBIT)
  ...
  game.analogReadPosition = actualPotRead;
  gameOnKeyDown(&game, EVENT_MOVE_ANALOG);
}
...
{
  pinMode(A5, INPUT);
  lastPotRead = map(analogRead(A5), 0, 1024, -5, 15);
  pinMode(2, INPUT);
  pinMode(4, INPUT);
}
