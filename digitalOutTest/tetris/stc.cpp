    ...
    if (game->isOver != 0) {
        ...
        if (game->isPaused != 0) {
            ...
            if ((game->data->events != EVENT_NONE) != 0) {
                ...
                if ((game->data->events & EVENT_MOVE_ANALOG) != 0) {
                    int amountToMove = game->analogReadPosition - game->fallingBlock.x;
                    moveTetromino(game, amountToMove, 0);
                }else if ((game->data->events & EVENT_MOVE_RIGHT) != 0) {
...
#endif
        break;
    case EVENT_MOVE_ANALOG:
        game->data->events |= EVENT_MOVE_ANALOG;
        break;
